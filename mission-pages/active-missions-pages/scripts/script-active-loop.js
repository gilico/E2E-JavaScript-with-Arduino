...
addEventListener('keydown', (event) => {
    ...
    if (event.keyCode == 84)
        ShowPoints();
    //if '3' is press from arduino - 1 round is finished
    if (event.keyCode == 51)
        FinishRound();
        //if 'q' is press from arduino - href to end mission page with the points
    if (event.keyCode == 81)
        closeTab();
...
function closeTab() {
    ...
    localStorage.setItem('getPoint3', JSON.stringify(points));
